...
dx=0.1;
vtildatable=alpha*dt*vtilda(0:dx:tracklength);
...
for t=1:npts
    for d=1:ndrivers
        ...
        if drivers(d).x>drivers((drivers(d).infront)).x || drivers(d).x<drivers((drivers(d).behind)).x
